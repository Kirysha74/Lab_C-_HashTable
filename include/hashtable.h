...
namespace hashtable {
	...
	public:
		...
		/*HashTable(bool flag) {
			...
			for (int i = 0; i < pow(2, _capacity); ++i) {
				_table.push_back(LinkedList(i));
			}
			int summa = 0;
			size_t size = rand() % (int)pow(2, 15);
			...
			print();
			cout << summa << " --- количество повторившихся элементов" << endl;
		}*/
		...
		unsigned int hash_function(unsigned int value) const {
			return (value * a % 65536 >> (16 - _capacity)); // 65536 - 2^16 взял длину битового слова 16
		}
	...
	size_t task() {
		...
		size_t summa = 0;
		a.print();
		for (LinkedList<Pair> elem : a.get_table()) {
			if (elem.size() - 1 > 1) {
				summa++;
			}
		}
